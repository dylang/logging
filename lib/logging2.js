...
Log.prototype = {
  ...
  useColor: function(value) {
    this._style.disable( !value );
    return this;
  },
  ...
  _writeLog: function(d, l) {
    ...
    var lvl = "";
    if ( l == 1 ) {
      lvl = this._style.color(Style.RED, "ERROR") + ": ";
    } else if ( l == 2 ) {
      lvl = this._style.color(Style.YELLOW, "WARN") + ": ";
    } else if ( l == 3 ) {
      lvl = this._style.color(Style.BLUE, "INFO") + ": ";
    } else {
      lvl = this._style.color(Style.WHITE, "LOG") + ": ";
    }
  ...
  error: function(){
    ...
    var str = record(arguments, this.file(), this._style);
...
function record(messages, file, style) {
      ...
      switch (typeof msg) {
          case 'string':
              ...
              if (length && msg[0] === '(' && msg[length - 1] === ')') {
                  output.push(style.color(Style.CYAN, msg.substring(1, length - 1)));
              } else if (length && msg[0] === '[' && msg[length - 1] === ']') {
                  output.push(style.color(Style.BLUE, msg.substring(1, length - 1)));
              } else if (length && msg[0] === '{' && msg[length - 1] === '}') {
                  output.push(style.color(Style.MAGENTA, msg.substring(1, length - 1)));
              } else if (length && msg[0] === '*' && msg[length - 1] === '*') {
                  output.push(style.color(Style.RED, msg));
              } else if (length && msg[0] === '!' && msg[length - 1] === '!') {
                  output.push(style.color(Style.RED_BOLD, msg));

              } else if (length && msg.substr(0, 7) === 'http://') {
                      output.push(style.color(Style.UNDERLINE, msg));
                  } else {
                  output.push(style.color(Style.WHITE, msg));
              }
              break;
          case 'number':
              output.push(style.color(Style.YELLOW, msg));
              break;
          case 'function':
              output.push(style.color(Style.YELLOW_BOLD, msg.toString()));
              break;
          default:
              msg = Util.inspect(msg, false, 6, !style.disabled);
              if (msg.length > 80) {
                  msg = ("\n" + msg).split(/\n/).join("\n    ");
              }
              output.push(msg);
  ...
  return niceTime + ' - '
    + (file ? '[' : '')
    + (file ? style.color(Style.GREEN, file) : '')
    + (file ? '] - ' : '')
    + output.join(' ').replace(/\n/g, '\n' + '            ');
